...
#project 3 version
@app.route('/adduser/<firstname>/<lastname>/<email>/<password>/<streetaddress>/<city>', methods=['GET', 'POST'])
def adduser(firstname=None,lastname=None,email=None,password=None,streetaddress=None,city=None):
    ...
    with con:
        ...
        sa=streetaddress
        ct=city
        cur.execute('insert into User(FirstName, LastName, Email, Password, StreetAddress, City, Admin) values ("%s","%s","%s","%s","%s","%s",0)'%(fn,ln,em,pw,sa,ct))
    return redirect(url_for('home'))
...
# project 3 version
@app.route('/addvenue/<venue>/<eventcapacity>', methods=['GET', 'POST'])
def addvenue(venue=None,eventcapacity=None):
    with con:
        cur=con.cursor()
        vn=venue
        ec=eventcapacity
...
#project 3 version
@app.route('/deletevenue/<venue>', methods=['GET', 'POST'])
def deletevenue(venue=None):
    with con:
        cur = con.cursor()
        vn = venue
        cur.execute("delete from Venues where Venue=('%s')"%(vn))
        return redirect(url_for('deleteuser'))
    return render_template('deletevenue.html', title='Delete Venue')
...
def deleteevent(eventid=None):
    with con:
        ...
        cur.execute("delete from events where PKEventID = %s"%(id))
        return redirect(url_for('deleteuser'))
    return render_template('deleteevent.html', title='Delete Event')
...
def joinevent(userid=None,eventid=None):
    with con:
        ...
        uid=userid
        eid=eventid
        cur.execute("insert into UsersEvent(FKUserID, FKEventID) values (%s,%s);"%(uid,eid))
        return redirect(url_for('home'))
    return render_template('joinevent.html', title='Join Event')
...
#project 3 version
@app.route("/eventhejoin/<userid>")
def eventhejoin(userid=None):
    with con:
        cur = con.cursor()
        id = userid
        result = cur.execute("select FieldName, EventDescription, TimeStart, TimeEnd from(events join UsersEvent on UsersEvent.FKEventID=events.PKEventID ) where FKUserID = %s;"%(id))
        if result > 0:
            userinfo= cur.fetchall()
            return render_template('eventhejoin.html', userinfo = userinfo)
        return render_template('eventhejoin.html',title='What Event I Join')
...
#project 3 version
@app.route("/searchevent/<venue>")
def searchevent(venue=None):
    with con:
        cur = con.cursor()
        ve = venue
        result = cur.execute("select * from events join Venues on PKVenueID=events.FKVenueID where FKVenueID=(select PKVenueID where Venue = '%s');"%(ve))
        if result > 0:
            userinfo = cur.fetchall()
            return render_template('searchevent.html', userinfo=userinfo)
    return render_template('searchevent.html', title='What Event in this Venue?')
